...
export default function Saveurs() {
  ...
  const addToCurrentPack = (productId: string) => {
    ...
    if (totalInPack < pack.size) {
      ...
      alert(`Votre ${pack.name} est déjà complet. Veuillez ajuster votre panier.`);
    }
  };

  const buySingleProduct = (productId: string) => {
    setSelectedPack({ id: 'single', name: 'À l\'unité', size: 1, price: 25 });
    setCustomPack({ [productId]: 1 });
    setIsCheckoutOpen(true);
  };

  return (
    <section className="py-[128px] px-[20px] md:px-[64px] bg-cream">
      <div className="text-center mb-[80px]">
        <span className="text-[12px] font-black tracking-[0.25em] uppercase text-gold mb-4 block">La Collection</span>
        <h2 className="font-serif text-[clamp(36px,5vw,64px)] font-black text-espresso">Nos 8 <em className="italic font-accent text-gold">Saveurs</em> Signature</h2>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-[1400px] mx-auto">
        {PRODUCTS.map((p, i) => (
          <motion.div 
            ...
            className="bg-white rounded-[32px] overflow-hidden border border-taupe/10 hover:-translate-y-3 hover:shadow-[0_40px_100px_rgba(26,77,92,0.12)] transition-all duration-500 ease-spring group cursor-pointer"
          >
            ...
            <div className="p-8">
              <div className="flex items-center justify-between mb-2">
                <div className="font-serif text-[22px] font-black text-espresso">{p.name}</div>
                <button 
              ...
              <p className="text-[14px] text-taupe leading-relaxed mb-6 font-medium">{p.description}</p>
              <div className="flex items-center justify-between pt-4 border-t border-taupe/5">
                <span className="px-4 py-1.5 bg-gold/10 rounded-full text-[10px] font-black text-gold tracking-widest">100% NATUREL</span>
                <button 
